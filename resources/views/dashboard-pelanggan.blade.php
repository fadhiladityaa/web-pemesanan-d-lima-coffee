        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-4 space-y-16">
                        ...
                        @foreach ($promos->take(1) as $promo)
                            <div
                                ...
                                <div class="relative z-10 p-6 text-white">
                                    ...
                                    <div
                                        class="flex items-center justify-between bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-3">
                                        <div>
            ...
            <div>
                <div class="flex items-end justify-between mb-8">
                    ...
                    <a href="{{ url('/menu') }}"
                        class="hidden md:flex items-center gap-2 text-sm font-bold text-[#4A3B32] hover:text-[#D4B595] transition">
                        Lihat Semua →
                    </a>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    @forelse($featuredProducts as $product)
                        <<<<<<< HEAD <div
                            class="group relative bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-[#F0EAE0] overflow-hidden hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 ease-out">
                            ...
                            <div class="h-64 overflow-hidden relative bg-[#F4F1EA]">
                                ...
                                <div
                                    class="absolute inset-0 bg-gradient-to-t from-[#2C241B]/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition duration-500">
                                </div>
                            </div>
                            ...
                            <div class="p-6 relative">
                                {{-- Floating Price --}}
                                <div
                                    class="absolute -top-6 right-6 bg-[#2C241B] text-[#D4B595] px-4 py-2 rounded-xl text-sm font-bold shadow-lg transform translate-y-2 group-hover:translate-y-0 transition duration-300">
                                    Rp {{ number_format($product->harga, 0, ',', '.') }}
                                </div>

                                <h3 class="font-serif text-xl font-bold text-[#2C241B] mb-2 line-clamp-1">
                                    {{ $product->name }}</h3>
                                <p class="text-[#8C7B70] text-sm mb-6 line-clamp-2 leading-relaxed h-10">
                                    {{ $product->deskripsi }}</p>

                                <button
                                    class="w-full py-3 rounded-xl border border-[#D4B595] text-[#4A3B32] font-bold text-sm hover:bg-[#D4B595] hover:text-[#2C241B] transition-colors duration-300 flex items-center justify-center gap-2">
            ...
            @empty
                ...
                =======
                <div
                    class="group bg-white rounded-[2rem] border border-[#F0EAE0] overflow-hidden hover:shadow-2xl hover:-translate-y-2 transition-all duration-500">
                ...
                @empty
                    <div class="col-span-3 py-12 text-center rounded-[2rem] border border-dashed border-[#D1C7BD] bg-[#FDFBF7]">
                        ...
                        <a href="{{ url('/menu') }}"
                            class="text-[#4A3B32] text-sm font-bold hover:underline mt-2 inline-block">Cek Daftar Menu
                            Lengkap</a>
                    </div>
            ...
            <<<<<<< HEAD {{-- SECTION 4: EDUKASI (Wide Card) --}} @if ($edukasi)
                <div
                    class="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-[#F0EAE0] flex flex-col md:flex-row gap-10 items-center">
                ...
                <div class="py-8 text-center">
                    <a href="{{ url('/menu') }}"
                        class="group relative inline-flex items-center justify-center px-8 py-4 bg-[#2C241B] text-[#D4B595] font-bold text-lg rounded-full overflow-hidden shadow-2xl hover:scale-105 transition-transform duration-300">
                        <span
                            class="absolute w-0 h-0 transition-all duration-500 ease-out bg-[#D4B595] rounded-full group-hover:w-56 group-hover:h-56 opacity-10"></span>
                        ...
                        @if ($edukasi)
                            <div>
                                ...
                                <div
                                    class="relative group cursor-pointer h-[400px] rounded-[2.5rem] overflow-hidden shadow-xl border border-[#F0EAE0]">
                                    <div class="absolute inset-0 bg-gray-800">
                                        @if ($edukasi->image)
                                            <img src="{{ asset('storage/' . $edukasi->image) }}"
                                                class="w-full h-full object-cover opacity-70 group-hover:scale-110 group-hover:opacity-60 transition duration-1000 ease-out">
                                        @else
                                            <img src="https://images.unsplash.com/photo-1447933601403-0c60e017bc32?q=80&w=1000"
                                                class="w-full h-full object-cover opacity-70 group-hover:scale-110 group-hover:opacity-60 transition duration-1000 ease-out">
                                        @endif
                                    </div>
                                    ...
                                    <div
                                        class="absolute top-6 left-6 bg-white/20 backdrop-blur-md border border-white/30 text-white text-xs font-bold px-4 py-1.5 rounded-full uppercase tracking-wider">
                                        {{ $edukasi->kategori ?? 'Knowledge' }}
                                    </div>

                                    <div
                                        class="absolute bottom-0 left-0 p-8 md:p-12 w-full transform translate-y-2 group-hover:translate-y-0 transition duration-500">
                                        <h3
                                            class="font-serif text-3xl md:text-5xl font-bold text-white mb-4 leading-tight group-hover:text-amber-50 transition">
                                            {{ $edukasi->judul }}
                                        </h3>

                                        <p
                                            class="text-gray-300 text-sm md:text-lg line-clamp-2 mb-6 max-w-2xl group-hover:text-white transition">
                                            {{ $edukasi->ringkasan }}
                                        </p>

                                        {{-- [FIXED] LINK KE LIVEWIRE EDUKASI --}}
                                        <a href="{{ route('edukasi') }}"
                                            class="inline-flex items-center gap-2 text-[#D4B595] font-bold uppercase tracking-widest text-sm border-b border-[#D4B595] pb-1 hover:text-white hover:border-white transition">
                                            Baca Selengkapnya
                                        </a>
                                    </div>
                                </div>
