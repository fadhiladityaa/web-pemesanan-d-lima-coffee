    ...
    <nav class="fixed w-full z-50 transition-all duration-300 bg-[#947257] shadow-xl border-b border-[#947257]"
        ...
        <div class="hidden mobile-menu md:hidden bg-white border-t">
            <div class="px-4 pt-2 pb-4 space-y-1">
                @guest
                    <a href="#home"
                        class="block py-3 px-3 text-base font-medium text-gray-700 hover:bg-gray-50 rounded-lg">Beranda</a>
                @endguest
                <a href="#about"
                    class="block py-3 px-3 text-base font-medium text-gray-700 hover:bg-gray-50 rounded-lg">Tentang</a>
                <a href="{{ auth()->check() ? route('menu') : '#menu' }}"
                    class="block py-3 px-3 text-base font-medium text-gray-700 hover:bg-gray-50 rounded-lg">Menu</a>
                @if (Route::has('login'))
                    @auth
                        <a href="{{ url('/dashboard') }}"
                            class="block w-full text-center mt-4 px-5 py-3 rounded-lg bg-primary text-white font-bold shadow">Dashboard</a>
                    @else
                        <div class="grid grid-cols-2 gap-3 mt-4">
                            <a href="{{ route('login') }}"
                                class="block px-5 py-3 text-center rounded-lg border border-primary text-primary font-bold hover:bg-primary-50">Login</a>
                            @if (Route::has('register'))
                                <a href="{{ route('register') }}"
                                    class="block px-5 py-3 text-center rounded-lg bg-primary text-white font-bold shadow">Register</a>
                            @endif
                        </div>
                    @endauth
                @endif
    ...
    <section id="home"
        ...
        <div class="relative z-10 text-center max-w-4xl mx-auto px-4 sm:px-6">
            <div data-aos="fade-up" data-aos-duration="1000">
                <h1
                    class="text-4xl sm:text-5xl md:text-7xl font-extrabold text-white mb-6 leading-tight hero-text-shadow">
                    Nikmati Kopi Terbaik <br> <span class="text-primary">D'Lima Coffee</span>
                </h1>
                <p
                    class="text-lg sm:text-xl md:text-2xl text-gray-200 mb-10 max-w-2xl mx-auto font-light leading-relaxed drop-shadow">
                    Rasakan kehangatan dan kenikmatan dalam setiap tegukan. Kopi pilihan nusantara yang disajikan dengan
                    penuh cinta.
                </p>
                <div class="flex flex-col sm:flex-row justify-center gap-4">
                    <a href="#menu"
                        class="px-8 py-4 rounded-full bg-primary text-white font-bold text-lg hover:bg-opacity-90 transition-transform transform hover:scale-105 shadow-xl">
                        Pesan Sekarang
                    </a>
                    <a href="#about"
                        class="px-8 py-4 rounded-full bg-transparent border-2 border-white text-white font-bold text-lg hover:bg-white hover:text-gray-900 transition-all">
                        Selengkapnya
                    </a>
                </div>
            </div>
    ...
    <section id="menu" class="py-24 bg-gray-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                @foreach ($menus as $menu)
                    <div class="bg-white rounded-3xl shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-3 group"
                        ...
                        <div class="h-64 overflow-hidden relative"> <!-- Taller image area -->
                            ...
                            @endif
                            <div
                                class="absolute top-4 right-4 bg-white/90 backdrop-blur-sm text-primary px-4 py-1.5 rounded-full text-sm font-bold shadow-sm">
                                Best Seller
                            </div>
                        </div>
                        <div class="p-8">
                            <h3 class="text-2xl font-bold text-gray-900 mb-2 truncate"
                                title="{{ $menu->nama_menu }}">{{ $menu->nama_menu }}</h3>
                            <p class="text-gray-500 text-sm mb-4 line-clamp-2 leading-relaxed">
                                {{ $menu->deskripsi_menu ?? 'Nikmati kesegaran menu ini.' }}</p>
                            <a href="{{ route('detail.menu', $menu->id) }}"
                                class="text-sm text-primary font-semibold hover:text-primary-focus transition-colors flex items-center gap-1 group/link mb-4">
                                Lihat Detail
                            ...
                            <div class="flex justify-between items-center mt-auto">
                                ...
                                @auth
                                    <a href="{{ route('cart.buyNow', $menu->id) }}"
                                        class="w-12 h-12 rounded-full bg-primary flex items-center justify-center text-white hover:bg-primary-focus transition-all shadow-sm hover:shadow-md group-hover:bg-primary group-hover:text-white"
                                        title="Beli Sekarang">
                                ...
                                @else
                                    <a href="{{ route('login') }}"
                                        class="w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center text-primary hover:bg-primary hover:text-white transition-all shadow-sm hover:shadow-md group-hover:bg-primary group-hover:text-white"
            ...
            <div class="text-center mt-16">
                <a href="{{ route('menu') }}"
                    class="inline-block px-10 py-3 rounded-full bg-gray-900 text-white font-bold hover:bg-gray-800 transition-transform transform hover:scale-105 shadow-lg">
                    Lihat Semua Menu
                </a>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                ...
                <div class="relative" data-aos="fade-right" data-aos-duration="1000">
                    <div class="aspect-w-4 aspect-h-3 rounded-2xl overflow-hidden shadow-2xl">
                        <img src="{{ asset('img/hero.png') }}"
                            onerror="this.src='https://images.unsplash.com/photo-1554060851-f2fe77051df6?q=80&w=1887&auto=format&fit=crop'"
                            alt="About Us"
                            class="w-full h-full object-cover transform hover:scale-105 transition-transform duration-700">
                    </div>
                    <div
                        class="absolute -bottom-6 -right-6 md:-bottom-10 md:-right-10 bg-white p-6 md:p-8 rounded-xl shadow-xl border-l-[6px] border-primary hidden md:block animate-float">
                ...
                <div data-aos="fade-left" data-aos-duration="1000">
                    ...
                    <p class="text-gray-600 text-lg mb-6 leading-relaxed">
                        <span class="font-bold text-gray-900">D'Lima Coffee</span> adalah tempat di mana gairah kopi
                        bertemu dengan kenyamanan. Kami berkomitmen menyajikan kopi terbaik dari biji pilihan nusantara.
                    </p>
                    <p class="text-gray-600 text-lg mb-8 leading-relaxed">
                        Kami tidak hanya menjual kopi, kami menjual pengalaman. Sebuah ruang untuk produktivitas,
                    ...
                    <div class="space-y-4">
                        <div class="flex items-start">
                            <div
                                class="flex-shrink-0 h-6 w-6 rounded-full bg-primary/20 flex items-center justify-center mt-1">
