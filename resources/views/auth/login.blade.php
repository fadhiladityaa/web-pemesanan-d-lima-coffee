...
<body class="font-[Poppins] min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-[#2a1a10] via-[#5c3a21] to-[#C67C4E]">
    ...
    <!-- Card Wrapper -->
    <div class="relative w-full max-w-4xl bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row transform transition-all duration-300 hover:shadow-primary/20">
        
        <!-- Left Side: Image (Desktop Only) -->
        <div class="hidden md:block w-1/2 bg-cover bg-center relative group" 
             style="background-image: url('{{ asset('img/bg1.jpeg') }}');">
            <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent flex flex-col justify-end p-12 text-white transition-all duration-300 group-hover:bg-black/50">
                <h2 class="text-3xl font-bold mb-2 text-secondary">Welcome Back!</h2>
                <p class="text-sm opacity-90 leading-relaxed">Nikmati setiap tegukan kopi terbaik dari D'Lima Coffee. Masuk untuk melanjutkan pesananmu.</p>
            </div>
        </div>
        ...
        <div class="w-full md:w-1/2 p-8 md:p-12 lg:p-16 flex flex-col justify-center bg-white relative">
            ...
            @if(session('LoginError'))
                <div class="mb-6 p-4 rounded-xl bg-red-50 border border-red-200 text-red-600 text-sm flex items-center gap-3 shadow-sm" role="alert">
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="relative group">
                        ...
                        <input type="tel" name="noHp" placeholder="08xxxxxxxxxx" required maxlength="13" minlength="8"
                ...
                <p class="text-center text-sm text-gray-500 mt-6">
                    Belum punya akun? 
                    <a href="{{ route('register') }}" class="text-primary font-bold hover:text-[#A66236] transition-colors underline decoration-2 decoration-transparent hover:decoration-primary">Daftar disini</a>
                </p>
            </form>
        </div>
    </div>
